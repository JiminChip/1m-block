...
static struct ret_struct print_pkt (struct nfq_data *tb)
{
	...
	for (int i = 0; i < ret - 6; i++) {
		if (!strncmp(&(((struct http *)data)->data[i]), "Host: ", 6)) {
			//printf("%s\n", &(((struct http *)data)->data[i]));
			if(!search_hostname(&(((struct http *)data)->data[i]), black_list)) {
				//printf("harmful website detected - Blocked");
				ret_val.blocked = 1;
				puts("Dropped");
			}
		}
...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		...
		fprintf(stderr, "sample : netfilter-test top-1m.csv\n");
		exit(1);
	}
	...
	// alloc buffer for file name
	int file_len = strlen(argv[1]);
	char* site_filename = (char*)malloc(sizeof(char) * (file_len) + 1);
	strncpy(site_filename, argv[1], file_len);

	puts("make trie start");
	black_list = csv_to_trie((const char*) site_filename);
	puts("complete");
